...
Location: `src/test/java`. Run with `./gradlew test` (also part of `./gradlew build`). 620 tests in 40 classes:
...
timeout the suite would take **6,130 ticks (306.5 s at 20 TPS)**: the sum of the longest `timeoutTicks` in each of the
**40** batches (GS-116 added `gregscope.surface.ochub` at **40** - nine tests, one observed tick each - and
`gregscope.surface.ochub.script` at **450**, the OpenOS boot budget, for two tests of 147 observed ticks; GS-115
added `gregscope.surface.ocsensor` at 40 and `.ocsensor.reload` at 60) (GS-114 added `gregscope.surface.hubgui` at **20**, not the default 100: every test in it drives
`buildUI` and the named handlers inside one server tick and the report confirms 0 observed ticks for all eleven) (GS-110 left it at 4,320 over 30; GS-111 added `gregscope.surface.command`,
...
> the real 600 s step, today's worst case of **6,130 ticks = 306.5 s** plus that 30 s leaves about **260 s of
