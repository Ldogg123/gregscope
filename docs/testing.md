...
Location: `src/test/java`. Run with `./gradlew test` (also part of `./gradlew build`). 677 tests in 46 classes:
...
| `buffers.BufferCollectorTest` | 13 | GS-301, design-v0.3-buffers section 3. The bounded top-K rules: largest first, everything past four rolled into `other` with the **totals staying exact**, a displaced entry going to the rollup rather than vanishing. An empty tank still contributes its capacity, because "0 of 16,000" is the state worth showing; saturation is `NaN` when nothing reports a capacity and `0.0` when a real tank is empty, and the test pins that those are different. ME-backed inputs are counted and contribute no amount. A reused collector must carry nothing from the machine before - the sampler shares one instance across every machine, so a stale entry would attribute one machine's fluid to another. |
| `buffers.BufferLinesTest` | 7 | GS-301. The snapshot encoding, pinned because other people parse it: `key=amount/capacity`, largest first, then `other=<amount>x<count>` and `me=<count>`. A missing capacity is **omitted**, never written as `/0`. |
| `buffers.BufferTrendTest` | 9 | GS-304. Direction over a window, and the cases where the honest answer is "unknown". The important one is `gapSecondsAreSkippedNotCountedAsEmpty`: an unloaded chunk records gap seconds, and if those counted as zero every machine would look like it drained to empty the moment nobody was watching - the most obvious way this feature could cry wolf. Fewer than five measurable samples, or inputs that report no capacity at all, is `UNKNOWN` rather than a guess. The deadband is compared at the ring's own resolution: on raw doubles a 0.50-to-0.52 ramp decodes as 0.020000000000000018 and exactly-the-deadband read as a rise. |
...
timeout the suite would take **6,370 ticks (318.5 s at 20 TPS)**: the sum of the longest `timeoutTicks` in each of
the **43** batches. Recount it with `python tools/batch_budget.py`, which reads the `@GameTest` annotations rather
...
| `gregscope.buffers` | 200 | GS-302; forms a real EBF and fills it, which takes real ticks. |
...
> the real 600 s step, today's worst case of **6,370 ticks = 318.5 s** plus that 30 s leaves about **250 s of
...
| `BufferProbeTests` | `gregscope.buffers` | GS-302. The buffer walk against a real formed EBF, and - the point of the class - **proof that it does not write to the machine**. `theWalkDoesNotWriteToTheMachine` clears every input hatch's `mRecipeMap`, runs the walk, asserts nothing moved, and then calls GT's own `getStoredFluids()` and asserts that it **does** set the field. That second half is what makes the first worth anything: the run reports "walk wrote nothing; getStoredFluids set mRecipeMap on 1 hatches", so the assertion is demonstrably able to catch a write rather than passing because nothing was ever going to change. It is also why GregScope walks the hatch lists instead of calling the obvious API. |
